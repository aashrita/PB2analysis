...
# CODE ADAPTED FROM: John's dump_timestreams.py
...
def get_TOD(frame, start, end, tod_buffs_I, tod_buffs_Q):
...
def get_times(frame, times_buff):
    if frame.type == core.G3FrameType.Scan:
        times_buff.append(np.array([t.time * 1.e-8 for t in frame['DetectorSampleTimes']]))

def get_azel(frame, antennatime_buff, az_buff, el_buff):
    if frame.type == core.G3FrameType.Scan:
...
# def plot_ADCvsTOD(bolodates, tod_I, tod_Q, antennadates, plotdir, az, el, run_number, det):
...
def dump(run_number, dir_output):
    ddir = '/data/pb2/ChileData/g3compressed/20000000/Run2000{:04d}/'.format(run_number)
